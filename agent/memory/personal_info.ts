  ...
  provider: defineMemoryProvider({
    ...
    async tools(context) {
      ...
      return {
        update: defineTool({
          description:
            "Update model-readable Personal Info after the user explicitly states or corrects reusable form information, including their timezone. Normalize timezone names to IANA identifiers such as America/New_York. This tool cannot read Personal Info; recalled values are already present in context. Pass null to remove a field. Never store credentials, payment details, tokens, or one-time codes.",
